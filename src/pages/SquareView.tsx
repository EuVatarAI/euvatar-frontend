  ...
  return (
    <div className="min-h-screen bg-gradient-subtle">
      ...
      <main className="container py-8 space-y-6">
        ...
        {/* Squares Accordion */}
        <Card className="gradient-card shadow-card border-border p-6">
          <h2 className="text-xl font-semibold mb-4">Selecione uma Praça</h2>
          <Accordion type="single" collapsible className="w-full">
            {mockSquares.map((square) => (
              <AccordionItem key={square.id} value={square.id}>
                <AccordionTrigger className="hover:no-underline">
                  <div className="flex items-center justify-between w-full mr-4">
                    <div className="flex items-center gap-3">
                      <MapPin className="h-5 w-5 text-primary" />
                      <div className="text-left">
                        <div className="font-semibold">{square.name}</div>
                        <div className="text-sm text-muted-foreground">{square.location}</div>
                      </div>
                    </div>
                    <Badge variant="outline" className="text-primary border-primary">
                      {square.totems.length} totens
                    </Badge>
                  </div>
                </AccordionTrigger>
                <AccordionContent>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
                    {square.totems.map((totem) => (
